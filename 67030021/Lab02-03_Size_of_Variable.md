...
1.2 จากผลลัพธ์ นักศึกษาสังเกตเห็นอะไรเมื่อค่าเกินขอบเขตของ int บน ESP32? 
    ค่าจะ วนกลับ ไปอีกฝั่ง (เรียกว่า Overflow หรือ Underflow) เช่น ถ้าเกิน 2,147,483,647 จะกลายเป็นค่าติดลบ
...
double มีความแม่นยำมากกว่า float (แสดงทศนิยมได้ละเอียดกว่า)
ควรใช้ double เมื่อ ต้องการความแม่นยำสูง เช่น คำนวณวิทยาศาสตร์ หรือข้อมูลพิกัด GPS
...
- char ใช้กี่ไบต์? ค่าตัวเลข (ASCII value) มีความสัมพันธ์กับอักขระอย่างไร?
    char ใช้ 1 ไบต์
    ASCII คือระบบที่ กำหนดเลขให้กับอักขระ เช่น 'A' = 65, 'Z' = 90, 'a' = 97
...
- บน ESP32, long มีขอบเขตเท่ากับ int หรือไม่? 
    เท่ากัน (ขนาด 4 ไบต์)
- ชนิดข้อมูลใดที่คุณจะใช้หากต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด?
...
byte ใช้กี่ไบต์? เมื่อ myByte ถูกกำหนดให้เป็น 256 ผลลัพธ์ที่ได้คืออะไร และเพราะเหตุใด?
    byte ใช้ 1 ไบต์ (8 บิต) เก็บได้ 0–255
    ถ้าใส่ 256 จะได้ผลลัพธ์เป็น 0 เพราะเกิด Overflow (วนรอบกลับไปเริ่มที่ 0)
...
ทำไม 10 / 3.0 (เมื่อตัวหารเป็น float หรือ double) ถึงได้ผลลัพธ์เป็นทศนิยม แต่เมื่อตัวหารถูกแปลงเป็น int แล้วผลลัพธ์เป็นจำนวนเต็ม?
    เพราะ 3.0 เป็น float/double → ผลลัพธ์จึงเป็นทศนิยม
    แต่ถ้าแปลงเป็น int → คำนวณแบบจำนวนเต็ม ทศนิยมจะถูกตัดทิ้ง
...
-ต้องการ ควบคุมชนิดผลลัพธ์ของการคำนวณ เช่น หารแล้วให้ได้ทศนิยม
-ต้องการ ตัดทศนิยมทิ้ง (แปลง float เป็น int)
-ต้องการดู รหัส ASCII ของอักขระ
-แปลงชนิดข้อมูลให้ ตรงกับชนิดที่ฟังก์ชันต้องการ
