...
2.1  สังเกตผลจากการรันโคิดในส่วน 2.1 ว่าตรงตามหลักการทางคณิตศาสตร์หรือไม่ เพราะเหตุใด
 ตรงตามหลักการทางคณิตศาสตร์ เพราะค่า 100 อยู่ในช่วงที่ int สามารถเก็บได้สบาย ๆ

2.2  สังเกตผลจากการรันโคิดในส่วน 2.2 ว่าตรงตามหลักการทางคณิตศาสตร์หรือไม่ เพราะเหตุใด
ตรงตามหลักการทางคณิตศาสตร์
เพราะ 2147483647 เป็นค่าสูงสุดของ int พอดี ยังไม่มีการ overflow

2.3  สังเกตผลจากการรันโคิดในส่วน 2.3 ว่าตรงตามหลักการทางคณิตศาสตร์หรือไม่ เพราะเหตุใด
 ไม่ตรงกับหลักการทางคณิตศาสตร์
เพราะตามหลักคณิตศาสตร์ ควรได้ 2147483648 แต่เนื่องจากมัน เกินขอบเขตของ int ระบบจึงเกิด Overflow — ค่าจะวนกลับไปที่ค่าต่ำสุดของ int (-2147483648)



2.4  สังเกตผลจากการรันโคิดในส่วน 2.4 ว่าตรงตามหลักการทางคณิตศาสตร์หรือไม่ เพราะเหตุใด
ตรงตามหลักการทางคณิตศาสตร์
เพราะ -2147483648 เป็นค่าต่ำสุดที่ int รองรับได้

2.5  สังเกตผลจากการรันโคิดในส่วน 2.5 ว่าตรงตามหลักการทางคณิตศาสตร์หรือไม่ เพราะเหตุใด
ไม่ตรงกับหลักการทางคณิตศาสตร์
เนื่องจากค่าที่ควรจะได้คือ -2147483649 ซึ่ง เกินขอบเขตล่างของ int ระบบจึงเกิด Underflow — ค่าจะวนกลับไปที่ค่าสูงสุดของ int (2147483647)
...
float (4 ไบต์) เก็บทศนิยมได้ประมาณ 6–7 ตำแหน่ง

ในตัวอย่าง: myFloat = 3.14159250 (ค่าถูกปัดทศนิยม)

double (8 ไบต์) เก็บทศนิยมได้ประมาณ 15–16 ตำแหน่ง

ในตัวอย่าง: myDouble = 3.141592653589793 (แม่นยำมากกว่า)

➡️ จึงเห็นได้ว่า double ให้ค่าที่แม่นยำกว่ามาก โดยเฉพาะในการเก็บค่าทศนิยมจำนวนมาก เช่น ค่าคงที่ทางคณิตศาสตร์ วิศวกรรม หรือวิทยาศาสตร์
3.2 สถานการณ์ใดที่คุณควรเลือกใช้ double แทน float?
คุณควรเลือกใช้ double แทน float เมื่อ:

✅ ต้องการความแม่นยำสูง เช่น:

การคำนวณฟิสิกส์ / วิศวกรรม

ระบบนำทาง (GPS) หรือพิกัดที่ต้องการความละเอียดระดับเซนติเมตร

การประมวลผลภาพ หรือเสียงละเอียดสูง

อัลกอริธึมคณิตศาสตร์ที่ซับซ้อน เช่น Fourier Transform, Machine Learning

✅ ต้องเปรียบเทียบค่าทศนิยมที่แตกต่างกันเพียงเล็กน้อย

float อาจให้ค่าคลาดเคลื่อนที่มีผลต่อผลลัพธ์

❗ หลีกเลี่ยงใช้ในระบบที่ หน่วยความจำหรือประสิทธิภาพสำคัญมาก เพราะ double ใช้พื้นที่และเวลาในการประมวลผลมากกว่า
...
ค่าตัวเลขของ char คือ รหัส ASCII ซึ่งเป็นมาตรฐานที่กำหนดว่า ตัวเลข 0–127 (ASCII มาตรฐาน) แต่ละค่าจะตรงกับอักขระใด เช่น:
...
➡️ ทุกตัวอักษรที่เห็นบนจอคอมพิวเตอร์ล้วนถูกแทนด้วยตัวเลขเบื้องหลัง
เช่น myChar = 'Z'; มีค่า 90 เมื่อแปลงเป็น int
และ myChar = 122; จะแสดงผลเป็น 'z'
...
แหล่งอ้างอิงที่นิยม เช่น:

เว็บไซต์:

ASCII Table (ascii-code.com)
...
Datasheet ของไมโครคอนโทรลเลอร์


4.3 จากข้อ 4.2 นักเขียนโปรแกรมสามารถกำหนดขึ้นเองได้ หรือมีเอกสารใดกำกับอยู่
ไม่สามารถกำหนดขึ้นเองได้ในกรณีใช้ char กับ ASCII
เพราะ ASCII เป็น มาตรฐานสากลที่ถูกกำกับโดย ANSI (American National Standards Institute)
และใช้ทั่วโลกในการแปลงข้อมูลอักขระในระบบคอมพิวเตอร์

หากต้องการกำหนดชุดอักขระของตัวเอง สามารถทำได้ในรูปแบบ:

ตารางจับคู่ (lookup table)

รหัส Unicode ที่สามารถกำหนดอักขระมากกว่าระบบ ASCII

แต่ถ้ายังคงใช้ char 8 บิตปกติ (เช่นใน ESP32 หรือ Arduino) จะใช้ ASCII เป็นหลัก เว้นแต่จะใช้ระบบอื่นที่รองรับ Unicode โดยตรง เช่น UTF-8 ผ่าน Library เสริม
...
false จะแสดงเป็น 0
เพราะว่า:
ในภาษา C/C++ (รวมถึง Arduino/ESP32), ตัวแปรชนิด bool เก็บค่าเป็นตัวเลข:

true คือ ค่าจริง หรือ เลข 1

false คือ ค่าเท็จ หรือ เลข 0

เมื่อใช้ Serial.println() กับตัวแปร bool — ค่าจะถูกพิมพ์ออกมาเป็นตัวเลข 1 หรือ 0 แทนคำว่า true / false
...
บน ESP32, long มีขอบเขตเท่ากับ int คือ 32-bit
6.2 ชนิดข้อมูลใดที่ต้องใช้หากต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด?
ถ้าต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด ให้ใช้
 unsigned long long (ขนาด 64-bit แบบไม่มีเครื่องหมาย) 
