...
ตรงตามหลักการทางคณิตศาสตร์ เพราะกำหนดค่า 100 ให้กับตัวแปร myInteger ซึ่งเป็นค่าภายในขอบเขตของ int (32-bit)

2.2  สังเกตผลจากการรันโคิดในส่วน 2.2 ว่าตรงตามหลักการทางคณิตศาสตร์หรือไม่ เพราะเหตุใด
ตรงตามหลักการ เพราะ 2147483647 คือค่าสูงสุดของ int แบบ signed บน ESP32 (32-bit) ดังนั้นค่าที่แสดงออกมาจึงถูกต้อง

2.3  สังเกตผลจากการรันโคิดในส่วน 2.3 ว่าตรงตามหลักการทางคณิตศาสตร์หรือไม่ เพราะเหตุใด
ไม่ตรงตามหลักการทางคณิตศาสตร์ — 2147483647 + 1 ควรได้ 2147483648 แต่เกิด overflow ทำให้ค่ากลับไปเป็นค่าลบ

2.4  สังเกตผลจากการรันโคิดในส่วน 2.4 ว่าตรงตามหลักการทางคณิตศาสตร์หรือไม่ เพราะเหตุใด
ตรงตามหลักการ — -2147483648 คือค่าต่ำสุดที่ int บน ESP32 รองรับได้

2.5  สังเกตผลจากการรันโคิดในส่วน 2.5 ว่าตรงตามหลักการทางคณิตศาสตร์หรือไม่ เพราะเหตุใด
ไม่ตรงตามหลักการทางคณิตศาสตร์ — -2147483648 - 1 จะเกิด underflow ค่าจะกลับไปเป็นค่าบวกสูงสุดของ int
...
สังเกตได้ว่า float (4 ไบต์) แสดงค่า pi ได้ประมาณ 6-7 ตำแหน่งทศนิยม ส่วน double (8 ไบต์) แสดงได้ถึง 15 ตำแหน่ง — แสดงถึงความแม่นยำที่สูงกว่าชัดเจน
3.2 สถานการณ์ใดที่คุณควรเลือกใช้ double แทน float?
ควรใช้ double เมื่อจำเป็นต้องเก็บหรือคำนวณค่าที่ต้องการ ความแม่นยำสูง เช่น งานวิทยาศาสตร์, ฟิสิกส์, หรือคณิตศาสตร์ขั้นสูง
...
ตัวเลข ASCII แต่ละค่ามีความสัมพันธ์กับตัวอักขระ เช่น 'Z' = 90, 'z' = 122 — โดย char เก็บค่าจำนวนเต็มของ ASCII ไว้
4.2 ถ้าอยากทราบความสัมพันธ์ระหว่างตัวเลขกับอักขระทั้งหมด สามารถหาได้จากเอกสารใด หรือแหล่งอ้างอิงใด
เอกสารอ้างอิง ASCII table สามารถหาได้จากเว็บไซต์ทั่วไป เช่น https://www.asciitable.com
4.3 จากข้อ 4.2 นักเขียนโปรแกรมสามารถกำหนดขึ้นเองได้ หรือมีเอกสารใดกำกับอยู่
ไม่สามารถกำหนดเองได้ — มีมาตรฐานกลางที่ชื่อว่า ASCII หรือ Unicode เป็นเอกสารกำกับ
...
บน Serial Monitor, true แสดงเป็น 1 และ false แสดงเป็น 0
...
บน ESP32 long มีขนาด 32-bit เท่ากับ int
6.2 ชนิดข้อมูลใดที่ต้องใช้หากต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด?
หากต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด ควรใช้ unsigned long long เพราะมีขนาด 64-bit ไม่มีเครื่องหมาย
...
ได้ค่า 0 เพราะ byte เก็บได้แค่ 0-255 (8-bit)
เมื่อใส่ 256 จะวนค่ากลับไปเริ่มใหม่ (เกิด overflow)
