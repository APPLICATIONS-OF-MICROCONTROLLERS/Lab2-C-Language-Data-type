...
ตอบ เมื่อกำหนดค่าที่ เกินขอบเขต, จะเกิด Integer Overflow หรือ Underflow ซึ่งทำให้ค่า วนกลับ (wrap around)
...
ตอบ ทั้ง float และ double บน ESP32 ใช้ขนาดเท่ากันคือ 4 ไบต์
- นักศึกษาสังเกตเห็นความแตกต่างของความแม่นยำระหว่าง float และ double อย่างไร? สถานการณ์ใดที่คุณควรเลือกใช้ double แทน float?
ตอบ บน ESP32, float กับ double มีขนาดและความแม่นยำเท่ากัน คือ 4 ไบต์ และแม่นยำประมาณ 6–7 หลักทศนิยม
เลือกใช้ double ก็ต่อเมื่อใช้บนแพลตฟอร์มที่รองรับ double 64 บิต และต้องการความแม่นยำสูงกว่าบน ESP32 ปกติ ใช้ float
...
ตอบ char ใช้ 1 ไบต์ (8 บิต) ในการเก็บข้อมูลค่าตัวเลขที่เก็บใน char คือ รหัส ASCII (ASCII value) ของอักขระนั้น ๆ
...
ตอบ bool ใช้ 1 ไบต์ ในการเก็บข้อมูล
...
ตอบ long = 4 ไบต์ (เหมือน int บน ESP32)
long long = 8 ไบต์
unsigned int = 4 ไบต์
...
ตอบ long บน ESP32 มีขอบเขตเท่ากับ int (32-bit)
ถ้าต้องการเก็บจำนวนเต็มบวกใหญ่ที่สุด ใช้ unsigned long long
...
ตอบ byte ใช้ 1 ไบต์ ในการเก็บข้อมูลเมื่อกำหนด myByte = 256; ผลลัพธ์ที่ได้คือ 0
เพราะ byte เก็บค่าได้แค่ 0–255 เท่านั้น ค่า 256 เกินขอบเขตจึงเกิด overflow และวนกลับมาเริ่มที่ 0 (256 mod 256 = 0)
...
ตอบ เมื่อตัวหารเป็น float หรือ double การหารจะเป็น การหารแบบทศนิยม (floating-point division) จึงได้ผลลัพธ์เป็นทศนิยม
แต่เมื่อตัวหารถูกแปลงเป็น int การหารจะเป็น การหารแบบจำนวนเต็ม (integer division) ซึ่งจะตัดทศนิยมออกและคืนค่าจำนวนเต็มเท่านั้น
...
ตอบ นักศึกษาควรใช้ Type Casting เมื่อต้องการแปลงข้อมูลจากชนิดหนึ่งเป็นอีกชนิดหนึ่ง เช่น float- int เพื่อเอาส่วนจำนวนเต็ม
