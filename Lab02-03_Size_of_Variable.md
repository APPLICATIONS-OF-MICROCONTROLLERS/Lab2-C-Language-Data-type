...
ตอบ 2147483647 + 1 → เกิด Overflow กลับไปเป็นค่าติดลบ -2147483648
    -2147483648 - 1 → เกิด Underflow กลับไปเป็น 2147483647
...
ตอบ float = 4 ไบต์ 32 บิต
    double = 8 ไบต์ 64 บิต
- นักศึกษาสังเกตเห็นความแตกต่างของความแม่นยำระหว่าง float และ double อย่างไร? สถานการณ์ใดที่คุณควรเลือกใช้ double แทน float?
ตอบ float แสดงทศนิยมได้ประมาณ 6–7 ตำแหน่ง
    double แสดงทศนิยมได้ประมาณ 15–16 ตำแหน่ง
...
ตอบ bool ใช้ 1 ไบต์
    ค่าที่แสดงใน Serial Monitor true = 1 false = 0
...
ตอบ บน ESP32, long มีขนาด เท่ากับ int 4 ไบต์ 
- บน ESP32, long มีขอบเขตเท่ากับ int หรือไม่? ชนิดข้อมูลใดที่คุณจะใช้หากต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด?
ตอบ ต้องการเก็บจำนวนเต็มบวกขนาดใหญ่มาก ควรใช้ unsigned long long
...
ตอบ byte = 1 ไบต์ byte เก็บค่าได้ในช่วง 0–255 เท่านั้น 8 บิต การกำหนด byte myByte = 256; จะทำให้  Overflow แบบวนกลับ
...
ตอบ 10 / 3.0 → int / float → ผลลัพธ์เป็น float 10 / (int)3.0 → int / int → ผลลัพธ์เป็น int ตัดทศนิยมทิ้ง
...
ตอบ ควบคุมผลลัพธ์ของการหาร
แปลงค่าทศนิยมเป็นจำนวนเต็ม
ดูค่า ASCII ของตัวอักษร
