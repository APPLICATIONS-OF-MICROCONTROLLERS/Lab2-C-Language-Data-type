...
int บน ESP32 ใช้ 4 ไบต์ (bytes)

1.2 จากผลลัพธ์ นักศึกษาสังเกตเห็นอะไรเมื่อค่าเกินขอบเขตของ int บน ESP32? 
Overflow → ค่าจะวนกลับไปเริ่มที่ค่าต่ำสุด Underflow → ค่าจะวนกลับไปที่ค่าสูงสุด
...
float ใช้ 4 ไบต์อdouble ใช้ 8 ไบต์

- นักศึกษาสังเกตเห็นความแตกต่างของความแม่นยำระหว่าง float และ double อย่างไร? สถานการณ์ใดที่คุณควรเลือกใช้ double แทน float?
- 
float แม่นยำพอสำหรับงานทั่วไป, แต่ถ้าต้องการ ความแม่นยำทางคณิตศาสตร์หรือวิทยาศาสตร์, ควรใช้ double
...
char ใช้ 1 ไบต์ (1 byte) (แสดงจาก sizeof(char) ที่ผลลัพธ์คือ 1)
ค่าตัวเลข (ASCII value) เป็นรหัสที่แทนอักขระในระบบ ASCII เช่น
'Z' มีค่า ASCII เป็น 90
ตัวเลข 122 เมื่อกำหนดให้ char จะแแสดงเป็นอักขระ 'z' (เพราะ ASCII 122 คือ 'z')
...
ชนิดข้อมูล bool บน ESP32 มีขนาด 1 ไบต์ และเมื่อพิมพ์ค่าของมันออกมา true จะถูกแสดงเป็น 1 ส่วน false จะถูกแสดงเป็น 0 บน Serial Monitor
...
long ใช้ 4 ไบต์
long long ใช้ 8 ไบต์
unsigned int ใช้ 4 ไบต์
unsigned long ใช้ 4 ไบต์
unsigned long long ใช้ 8 ไบต์


- บน ESP32, long มีขอบเขตเท่ากับ int หรือไม่? ชนิดข้อมูลใดที่คุณจะใช้หากต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด?

ใช่  บน ESP32 ทั้ง long และ int เป็นชนิดข้อมูล 32-bit ที่มีขอบเขตค่าเท่ากัน 
(ประมาณ ±2,147,483,647 สำหรับ signed)

ชนิดข้อมูลใดที่ควรใช้ถ้าต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด?

ควรใช้ unsigned long long (64-bit unsigned) เพราะเก็บค่าจำนวนเต็มบวกได้สูงสุดถึงประมาณ 18,446,744,073,709,551,615 ซึ่งใหญ่กว่า unsigned long (32-bit) มาก
...
byte ใช้ 1 ไบต์ (8 บิต) ในการเก็บข้อมูลเมื่อ myByte ถูกกำหนดเป็น 256 ผลลัพธ์ที่ได้คือ 0
เพราะค่า 256 เกินขอบเขตของ byte ที่เก็บได้แค่ 0–255 จึงเกิด overflow โดยค่าจะถูกวนกลับ 
(modulo 256) ทำให้ 256 % 256 = 0 ผลลัพธ์จึงเป็น 0
...
10 / 3.0 (ตัวหารเป็น float หรือ double) ได้ผลลัพธ์เป็นทศนิยม
เพราะการหารเกิดขึ้นระหว่างจำนวนเต็ม (int) กับจำนวนทศนิยม (float หรือ double) ซึ่งในขั้นตอนการคำนวณ ตัวแปร int จะถูกแปลงเป็นชนิดทศนิยมก่อนโดยอัตโนมัติ ทำให้ผลลัพธ์เป็นจำนวนทศนิยมที่แม่นยำกว่า 
เช่น 3.33 หรือ 3.3333333
แต่เมื่อ ตัวหารถูกแปลงเป็น int เช่น (int)3.0 กลายเป็นจำนวนเต็ม 3
การหารจึงเป็น int / int ซึ่งผลลัพธ์จะถูกเก็บในชนิดจำนวนเต็ม (int) และทศนิยมจะถูก ตัดทิ้ง (ไม่ปัดขึ้น) 
เช่น 10 / 3 = 3
...
เมื่อต้องการหลีกเลี่ยงข้อผิดพลาดหรือพฤติกรรมที่ไม่ต้องการจากการแปลงชนิดอัตโนมัติของภาษา
เช่น การแปลงชนิดข้อมูลอย่างชัดเจนเพื่อลดความสับสน หรือควบคุมผลลัพธ์ที่ได้
เมื่อต้องการทำงานกับข้อมูลในระดับบิตหรือหน่วยความจำ เช่น การแปลง pointer หรือข้อมูลดิบ
เช่น การแปลงชนิด pointer เพื่อเข้าถึงข้อมูลในรูปแบบต่างกัน
