...
เมื่อค่าเกินขอบเขต จะเกิด overflow ทำให้ค่ากลับไปเป็นค่าลบ
...
ควรใช้ double เมื่อ ต้องการความแม่นยำมากกว่า และพื้นที่ในหน่วยความจำไม่ใช่ปัญหา
...
ใช้ 1 ไบต์ ตัวเลข ASCII คือค่าที่ char แทนอยู่ เช่น 'A' = 65
...
ใช้ 1 ไบต์ (แม้ข้อมูลจริงจะมีแค่ 0 หรือ 1) true = 1, false = 0 บน Serial Monitor
...
ถ้าต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด → ใช้ unsigned long long
...
ใช้ 1 ไบต์ เมื่อกำหนดเป็น 256 จะกลายเป็น 0 เนื่องจาก overflow (วนค่ากลับ)
...
เพราะเมื่อหนึ่งในตัวเลขที่ใช้ในการหารเป็น float หรือ double การดำเนินการหารจะเป็น การหารแบบทศนิยม (Floating-point division) ซึ่งจะรักษาค่าทศนิยมไว้ในผลลัพธ์
...
นักศึกษาจะใช้การทำ Type Casting ในสถานการณ์ใดบ้างในการเขียนโปรแกรม?
 1. เพื่อให้ผลลัพธ์ทางคณิตศาสตร์ถูกต้อง เช่น การหารที่ต้องการผลลัพธ์เป็นทศนิยม
int a = 5, b = 2;
float result = (float)a / b;  // ไม่เช่นนั้นจะได้แค่ 2 แทนที่จะเป็น 2.5
เช่น float → int, char → int, int → float
2. เมื่อแปลงค่าระหว่างชนิดข้อมูลที่ต่างกัน
float temp = 27.8;
int rounded = (int)temp;  // ปัดเศษทิ้ง เหลือแค่ 27
3. เพื่อแสดงผลหรือทำงานกับค่าที่ต้องใช้ชนิดข้อมูลเฉพาะ
เช่น แปลง char เป็น int เพื่อดู ASCII
char c = 'A';
int ascii = (int)c;  // ได้ค่า 65
4. เพื่อลดขนาดหน่วยความจำ (Memory Optimization)
เช่น แปลง int เป็น byte เมื่อรู้ว่าใช้ค่าต่ำกว่า 255
5. ใช้ในกรณีที่ compiler ไม่สามารถแปลงอัตโนมัติได้
เช่น แปลง double ไป int อย่างชัดเจน เพื่อไม่ให้เกิด warning
6. เมื่อทำงานกับอุปกรณ์หรือโปรโตคอลที่ต้องการชนิดข้อมูลเฉพาะ
เช่น ส่งข้อมูลผ่าน Serial หรือเขียนค่าลง Register
