...
ตอบ ใช้ 4 ไบต์ (32 บิต)
1.2 จากผลลัพธ์ นักศึกษาสังเกตเห็นอะไรเมื่อค่าเกินขอบเขตของ int บน ESP32? 
ตอบ เมื่อค่าที่กำหนดเกินขอบเขตของ int (เช่น 2147483647 + 1) จะเกิด Overflow และวนกลับไปค่าติดลบ 
...
ตอบ float ใช้: 4 ไบต์
double ใช้: 8 ไบต์
- นักศึกษาสังเกตเห็นความแตกต่างของความแม่นยำระหว่าง float และ double อย่างไร? สถานการณ์ใดที่คุณควรเลือกใช้ double แทน float?
ตอบ ความแตกต่าง:
float แสดงทศนิยมได้ประมาณ 6–7 ตำแหน่ง
double แสดงได้ละเอียดประมาณ 15–16 ตำแหน่ง
ควรใช้ double เมื่อ:
ต้องการความแม่นยำสูง เช่น ค่าทางวิทยาศาสตร์, คำนวณฟิสิกส์, การวัดพิกัดที่ละเอียด
...
ตอบ char ใช้: 1 ไบต์
ความสัมพันธ์ ASCII:
อักขระแต่ละตัวแทนด้วยเลข ASCII เช่น 'A' = 65, 'z' = 122
สามารถแปลงไป-กลับระหว่าง char และ int ได้
...
ตอบ long มีขอบเขตเท่ากับ int บน ESP32 (ทั้งคู่เป็น 32-bit)
- บน ESP32, long มีขอบเขตเท่ากับ int หรือไม่? ชนิดข้อมูลใดที่คุณจะใช้หากต้องการเก็บค่าจำนวนเต็มบวกที่ใหญ่ที่สุด?
ตอบ หากต้องการเก็บค่าจำนวนเต็มบวกขนาดใหญ่มาก → ควรใช้ unsigned long long
...
ค่าที่เก็บได้: 0 – 255
เมื่อกำหนดเป็น 256:
จะเกิด Overflow → ค่าจะวนกลับไป 0 เพราะ 256 เกินขอบเขตของ 8 บิต
...
ตอบ เมื่อ int หารกับ float หรือ double → ค่าจะถูกแปลงอัตโนมัติเป็นชนิดที่แม่นยำกว่า → ได้ ผลลัพธ์เป็นทศนิยม
เมื่อแปลงเป็น int แล้วหาร → จะได้ จำนวนเต็ม (ทศนิยมถูกตัดทิ้ง)
...
ตอบ  1.ต้องการ ผลลัพธ์เป็นทศนิยม จากการหาร เช่น (float)a / b
2.แปลง ค่าทศนิยมเป็นจำนวนเต็ม เช่น (int)temperature เพื่อตัดทศนิยม
3.แปลง char เป็น ASCII หรือกลับกัน เช่น (int)'P' → 80
4.ทำให้ชนิดข้อมูล ตรงกับชนิดของฟังก์ชัน หรือการประมวลผล
5.ทำงานกับ pointer หรือ memory address (ใน C/C++ ขั้นสูง)
